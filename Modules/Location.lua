    ...
    if MS.DB.global.ShowLocationFrame then
        ...
        if MS.DB.global.LocationOnMouseover then
            Minimap:SetScript("OnEnter", function()
                UIFrameFadeRemoveFrame(MS.LocationFrame)
                MS.LocationFrame:SetAlpha(1)
                MS.LocationFrame:Show()
            end)
            Minimap:SetScript("OnLeave", function()
                UIFrameFadeRemoveFrame(MS.LocationFrame)
                UIFrameFadeOut(MS.LocationFrame, 0.5, MS.LocationFrame:GetAlpha(), 0)
                C_Timer.After(0.5, function()
                if MS.LocationFrame:GetAlpha() == 0 then MS.LocationFrame:Hide() end
                end)
            end)
